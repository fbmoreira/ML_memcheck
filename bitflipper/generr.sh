...
init_off=$(grep ELFFILE -e ".text" | awk '{print $5}')  		#obtains offset of .text in the binary
init_offset=$((16#$init_off)) 
...
#		./hackbin $@ | tee resultFLIP
		ps=${PIPESTATUS[0]}							#pipe status of binary execution					
                ...
                then
					...
					#echo $aux
					#checkvar=$(grep UNSUCC resultFLIP) 				#use diff for regular benchmarks, grep for UNSUCCESSFULL on NAS parallel benchmarks
					if [ $aux -ne 0 ]
